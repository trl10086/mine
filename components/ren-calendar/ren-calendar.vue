<template>
	<view class="calendar-wrapper1">
		<u-toast ref="uToast" />
		<view class="header1" v-if="headerBar">
...
export default {
	...
	methods: {
		...
		selectOne(i, event) {
			...
			let selectD = new Date(date).getTime(); //选择时间磋
			let chooseTo = new Date(self.today.date).getTime(); //今日时间磋
			let curTime = new Date().getTime();
			...
			// console.log(75415,!(chooseTo <= selectD))
			// if (chooseTo  >= selectD) {
			// 	// console.log(selectD < curTime,selectD <= curTime,selectD,curTime)
			//     if (true) {
			//         console.log('未来日期不可选');
			//         return false;
			//     } else  {
			...
			//         this.choose = date;
			// 		console.log(454512,selectD)
			//         this.$emit('onDayClick', response);
			//     }
			// } else {
			//     this.choose = date;
			//     this.$emit('onDayClick', response);
			// 	console.log(651424,this.choose,new Date(this.choose).getTime())
			// }
